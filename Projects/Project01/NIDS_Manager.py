...
class NIDS_Manager:
    ...
    def train_test_split(self):
        self.x_train, self.x_test, self.y_attack_train, self.y_attack_test, self.y_label_train, self.y_label_test = train_test_split(self.x, self.y_attack_cat, self.y_label, test_size=0.3, random_state=1)
    ...
    #feature analysis (Recursive Feature Elimination with Logistical Regression
    def rfecv(self):
        fa = FeatureAnalysis()
        
        fa.rfecv_fit(self.x_train, self.y_attack_cat)
        fa.rfecv_fit(self.x_train, self.y_label_train)
        
        fa.rfecv_eliminated_x(nids.x)